...
export default function Home() {
  ...
  const searchPdf = (e: React.ChangeEvent<HTMLInputElement>) =>{
    ...
    console.log(value)
    let filteredArray = pdfList.filter((item,index) => item.fileName?.toLowerCase().startsWith(value.toLowerCase()));
    ...
    <main className={styles.main}>
      ...
      <div className={styles.pdfListContainer}>
        ...
        <div className={styles.pdfListDiv}>
          {
            pdfList.length > 0 ? pdfList? pdfList.map((item,index )=> (
              <div key={index} style={{
                width: "100%",
                display: "flex",
                flexDirection: "row",
              ...
              <PdfListSkeleton />:<h3>PDF List Is Empty</h3> :
          }
        </div>
      </div>
    </main>
