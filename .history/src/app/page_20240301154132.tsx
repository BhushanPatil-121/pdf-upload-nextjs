...
export default function Home() {
  ...
  const searchPdf = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let filteredArray:any = pdfList.filter((item, index) => item.fileName?.toLowerCase().startsWith(value.toLowerCase()));
    setPdfList(filteredArray)
    if(value.length===0){
      setLoading(false);
    }
  }
    ...
    <main className={styles.main}>
      ...
      <div className={styles.pdfListContainer}>
        ...
        >
          ...
          />
          <IconButton type="button"  aria-label="search" onChange={searchPdf}>
            <SearchIcon />
          </IconButton>
        </Paper>
