...
export default function Home() {
  ...
  const [pdfList, setPdfList] = useState<Array<PdfList>>([])

  useEffect(() => {
    const getPdfList = async () => {
      const pdfList = await prisma.pdfTable.findMany()
      setPdfList(pdfList)
    }
  }, [])
  ...
  let list = new Array(40).fill("HDWUIHDHWDIH");
  return (
        ...
        <div className={styles.pdfListDiv}>
          {
           pdfList.length>0 ?  pdfList.map((item) => (
              <h2 className={styles.listTitle} key={item}>{item}</h2>
            )):<h2 className={styles.listTitle} >{item}</h2>
          }
        </div>
      </div>
