...
export default function Home() {
  ...
  const searchPdf = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearchPdfName(value)
    let filteredArray:any = pdfList.filter((item, index) => item.fileName?.toLowerCase().startsWith(searchPdfName.toLowerCase()));
    ...
    <main className={styles.main}>
      ...
      <div className={styles.pdfListContainer}>
        ...
        <div className={styles.pdfListDiv}>
          {
            searchList.length > 0 ? pdfList.map((item, index) => (
              <div key={index} style={{
                width: "100%",
                display: "flex",
                flexDirection: "row",
